...
- 태스크 실행 시 `dates × places` 조합을 만든 뒤, 조합 수만큼 동시에 예약을 시도합니다.
- 내부 동시 실행 수는 저장된 `concurrent` 값이 아니라 실제 조합 수로 자동 결정되며, 안전 상한은 100개입니다.
- `placeMode`가 `selected`이면 태스크에 지정된 장소만 실행하고, `all`이면 `data/places.json`의 전체 장소를 실행합니다.
- 스케줄 시작 시간이 과거이면 자동 실행을 등록하지 않습니다. 예약 오픈 시각 전에 서버가 실행 중이어야 합니다.
- 반복 실행은 기본 최대 10회이며, `runMode.maxRuns`가 있으면 1~10000 범위에서 그 값을 사용합니다.
- `burstRetry`가 켜져 있으면 각 조합마다 설정된 횟수/간격/최대 시간 안에서 재시도합니다. `retryNoRooms`가 켜져 있으면 방 없음 결과도 재시도 대상입니다.
